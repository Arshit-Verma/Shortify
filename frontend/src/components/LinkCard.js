...
import { copyToClipboard, deleteShortLink } from "../utils/api";
import "./LinkCard.css";
export const LinkCard = ({ link, onDelete }) => {
    ...
    const handleDelete = async () => {
        if (!window.confirm(`Delete link "${link.short_code}"? This cannot be undone.`)) {
            return;
        }
        setDeleting(true);
        try {
